...
export async function loadFromSupabase(): Promise<void> {
  ...
  try {
    ...
    // Supabase هو المصدر الرئيسي - لا نضيف مستخدمين محليين محذوفين من Supabase
    // فقط نضيف المستخدمين المحليين الذين لم يُرفعوا بعد (مثل admin أو مستخدم جديد جداً)
    for (const lu of localUsers) {
      const exists = appUsers.find((u: Record<string, unknown>) => u.id === lu.id || u.email === lu.email);
      if (!exists && lu.isAdmin) {
        // فقط Admin يُضاف محلياً إذا غير موجود في Supabase
        appUsers.push(lu);
      }
    }
...
async function pullFromSupabase() {
  ...
  try {
    ...
    // حذف المستخدمين المحليين الذين تم حذفهم من Supabase (ماعدا الأدمين)
    const sbIds = sbUsers.map((u: Record<string, unknown>) => u.id);
    const sbEmails = sbUsers.map((u: Record<string, unknown>) => u.email);
    const filteredUsers = localUsers.filter((lu: Record<string, unknown>) => {
      if (lu.isAdmin) return true; // لا تحذف الأدمين
      return sbIds.includes(lu.id) || sbEmails.includes(lu.email);
    });
    
    if (filteredUsers.length !== localUsers.length) {
      changed = true;
      console.log('[Supabase] 🗑️ تم حذف', localUsers.length - filteredUsers.length, 'مستخدم محذوف من Supabase');
    }

    if (changed) {
      originalSetItem.call(localStorage, 'cm_users', JSON.stringify(filteredUsers));
      // تحديث hash حتى لا يُعاد رفعها فوراً
      lastHash['users'] = hash(JSON.stringify(filteredUsers));
    }
...
async function syncUsers() {
  ...
  try {
    ...
    // تحقق من المستخدمين الموجودين في Supabase قبل الرفع
    let sbExistingIds: string[] = [];
    try {
      const checkRes = await fetch(`${SB_URL}/rest/v1/mining_users?select=id`, { headers: readHeaders });
      if (checkRes.ok) {
        const existing = await checkRes.json();
        if (Array.isArray(existing)) {
          sbExistingIds = existing.map((u: Record<string, unknown>) => u.id as string);
        }
      }
    } catch { /* صامت */ }

    // رفع فقط المستخدمين الموجودين في Supabase + الجدد (الأدمين أو المسجلين حديثاً)
    const filteredUsers = users.filter((u: Record<string, unknown>) => {
      if (u.isAdmin) return true; // دائماً ارفع الأدمين
      if (sbExistingIds.length === 0) return true; // إذا فشل التحقق، ارفع الكل
      return sbExistingIds.includes(u.id as string); // فقط الموجودين في Supabase
    });

    const rows = filteredUsers.map((u: Record<string, unknown>) => ({
      id: u.id,
      username: u.username,
      email: u.email,
      password: u.password || '',
