...
export async function loadFromSupabase(): Promise<void> {
  ...
  try {
    ...
    // دمج المستخدمين المحليين - الأحدث يفوز
    for (const lu of localUsers) {
      const existingIdx = appUsers.findIndex((u: Record<string, unknown>) => u.id === lu.id || u.email === lu.email);
      if (existingIdx >= 0) {
        // المستخدم موجود في كلا المكانين - احفظ الرصيد الأعلى والبيانات الأحدث
        const sbUser = appUsers[existingIdx] as Record<string, unknown>;
        const localBalance = Number(lu.balance) || 0;
        const sbBalance = Number(sbUser.balance) || 0;
        const localLogin = Number(lu.lastLogin) || 0;
        const sbLogin = Number(sbUser.lastLogin) || 0;
        
        // إذا البيانات المحلية أحدث، استخدمها
        if (localLogin > sbLogin) {
          appUsers[existingIdx] = {
            ...lu,
            balance: Math.max(localBalance, sbBalance),
          };
        } else {
          // بيانات Supabase أحدث لكن احفظ الرصيد الأعلى
          (appUsers[existingIdx] as Record<string, unknown>).balance = Math.max(localBalance, sbBalance);
          (appUsers[existingIdx] as Record<string, unknown>).password = lu.password || sbUser.password;
        }
      } else {
        appUsers.push(lu);
      }
