...
import { translations, type Language, rtlLanguages, languageNames } from './translations';

// ===== Translation Hook =====
function useTranslation() {
  const [lang, setLang] = useState<Language>(() => (store.getLanguage() as Language) || 'ar');
...
function AuthScreen({ onLogin }: { onLogin: (user: User) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRegister) {
      if (!username || !email || !password) { setError(t.fillAllFields); setLoading(false); return; }
      if (password.length < 6) { setError(t.passwordMismatch); setLoading(false); return; }
      const res = store.register(username, email, password, referralCode || undefined);
      if (res.success && res.user) { store.setLanguage(lang); onLogin(res.user); }
      else setError(res.error || t.registerError);
    } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-20">
        <select value={lang} onChange={e => changeLanguage(e.target.value as Language)}
          className="bg-gray-800/50 border border-gray-700/50 rounded-xl py-2 px-3 text-white text-xs focus:outline-none focus:border-blue-500/50 transition-all appearance-none cursor-pointer backdrop-blur-md">
          {Object.entries(languageNames).map(([code, name]) => (
            <option key={code} value={code} className="bg-gray-900 text-white">{name}</option>
          ))}
        </select>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md animate-scale-in">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2 text-sm">AI-Powered Cloud Mining Platform</p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {isRegister && (
              <div className="animate-slide-up">
                <label className="block text-xs text-gray-500 mb-1.5">{t.username}</label>
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">{t.email}</label>
              <div className="relative">
            ...
            {isRegister && (
              <div className="animate-slide-up">
                <label className="block text-xs text-gray-500 mb-1.5">{t.referralCode}</label>
                <div className="relative">
            ...
            {isRegister && (
              <div className="animate-slide-up">
                <label className="block text-xs text-gray-500 mb-1.5">{t.language}</label>
                <div className="relative">
          ...
          {isRegister && (
            <div className="mt-4 bg-gradient-to-r from-yellow-500/5 to-amber-500/5 border border-yellow-500/20 rounded-xl p-3 flex items-center gap-3 animate-slide-up">
              ...
              <p className="text-xs text-yellow-300/80">{t.welcomeStep2} - {t.welcomeProfit} $5</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      ...
      <div className="relative glass-card-solid rounded-3xl p-8 max-w-sm w-full animate-scale-in text-center" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-8">
          ...
          <p className="text-gray-400 text-sm mb-4">{t.welcomeSubtitle}</p>
          ...
          <div className="bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 rounded-2xl p-5 mb-4">
            ...
            <p className="text-yellow-300 font-bold text-lg mb-1">{t.gift}! 🎁</p>
            <p className="text-gray-300 text-sm">{t.giftDevice}</p>
            <p className="text-gray-300 text-sm">{t.welcomeDuration}</p>
            <p className="text-green-400 font-bold text-2xl mt-3 mining-profit-counter">$5.00</p>
            <p className="text-gray-500 text-xs mt-1">{t.welcomeProfit}</p>
          </div>
          ...
          <div className="space-y-2 mb-5">
            {step >= 1 && (
              <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/20 rounded-xl px-3 py-2 animate-slide-up">
                ...
                <p className="text-green-300 text-xs">{t.welcomeStep1}</p>
              </div>
            )}
            {step >= 2 && (
              <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-3 py-2 animate-slide-up">
                ...
                <p className="text-yellow-300 text-xs">{t.welcomeStep2}</p>
              </div>
            )}
            {step >= 3 && (
              <div className="flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 rounded-xl px-3 py-2 animate-slide-up">
                ...
                <p className="text-blue-300 text-xs">{t.welcomeStep3}</p>
              </div>
            )}
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 p-4 animate-fade-in" onClick={onClose}>
      ...
      <div className="relative glass-card-solid rounded-2xl w-full max-w-md max-h-[70vh] overflow-hidden animate-slide-down" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-gray-700/50 flex items-center justify-between bg-gradient-to-r from-blue-900/30 to-purple-900/30">
          <h3 className="font-bold text-white flex items-center gap-2"><Bell size={18} className="text-blue-400" /> {t.notifications}</h3>
    ...
    <div className="min-h-screen pb-24">
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-600">{t.welcome} {user.username}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <select value={lang} onChange={e => changeLanguage(e.target.value as Language)}
              className="bg-gray-800/30 border border-gray-700/30 rounded-lg py-1 px-1.5 text-white text-[10px] focus:outline-none cursor-pointer appearance-none">
              {Object.entries(languageNames).map(([code, name]) => (
      ...
      <main className="max-w-4xl mx-auto px-4 py-5">
        ...
        {page === 'my-devices' && (
          <div className="space-y-5 animate-slide-up">
            <div className="text-center">
              <h2 className="text-xl font-bold text-white mb-1">{t.myDevicesTitle} 🖥️</h2>
        ...
        {page === 'deposit' && (
          <div className="max-w-md mx-auto space-y-5 animate-slide-up">
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-white mb-1">{t.depositFunds}</h2>
        ...
        {page === 'withdraw' && (
          <div className="max-w-md mx-auto space-y-5 animate-slide-up">
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-white mb-1">{t.withdrawFunds}</h2>
            ...
            <div className="glass-card rounded-2xl p-5">
              ...
              <div className="bg-blue-500/5 border border-blue-500/15 rounded-xl p-4 mb-4">
                <h4 className="text-blue-300 text-xs font-bold mb-2 flex items-center gap-1.5">
                  <Shield size={14} /> {t.withdrawRulesTitle}
                </h4>
              ...
              <button onClick={handleWithdraw} disabled={!walletAddress.trim() || user.balance < 10 || (!!withdrawAmount && parseFloat(withdrawAmount) > user.balance)}
                ...
                {user.balance < 10 ? `❌ ${t.insufficientBalanceWithdraw}` : t.submitWithdraw}
              </button>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 glass border-t border-gray-800/30">
        <div className="max-w-4xl mx-auto flex items-center justify-around py-1.5 px-2">
          {[
            { id: 'dashboard' as Page, icon: Home, label: t.home },
            { id: 'store' as Page, icon: ShoppingCart, label: t.devices },
            { id: 'my-devices' as Page, icon: HardDrive, label: t.myDevices },
            { id: 'deposit' as Page, icon: Download, label: t.deposit },
            { id: 'withdraw' as Page, icon: Upload, label: t.withdraw },
          ].map(item => (
