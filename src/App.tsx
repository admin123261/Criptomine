...
function AdminChat({ onClose }: { onClose: () => void }) {
  ...
  const chatUsers = [...new Set(allMsgs.filter(m => m.from !== 'admin001' || m.to !== 'admin001').map(m => m.from === 'admin001' ? m.to : m.from))];
  ...
  return (
    <div className="glass-card-solid rounded-2xl overflow-hidden">
      ...
      <div className="flex h-80">
        <div className="w-1/3 border-l border-gray-700/40 overflow-y-auto hide-scrollbar" style={{ background: 'rgba(8, 12, 25, 0.95)' }}>
          {chatUsers.length === 0 && <p className="text-gray-500 text-xs p-3 text-center">لا توجد محادثات</p>}
          {chatUsers.map(uid => {
            const u = users.find(u => u.id === uid);
            const unread = allMsgs.filter(m => m.from === uid && m.to === 'admin001' && !m.read).length;
            return (
              <button key={uid} onClick={() => { setSelectedUser(uid); store.markMessagesRead(uid, 'admin001'); }}
                ...
                {unread > 0 && <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full ml-1">{unread}</span>}
              </button>
            );
          })}
        </div>
        <div className="flex-1 flex flex-col" style={{ background: 'rgba(5, 10, 20, 0.95)' }}>
          <div className="flex-1 overflow-y-auto p-3 space-y-2 hide-scrollbar">
            {!selectedUser && <p className="text-gray-500 text-xs text-center mt-8">اختر محادثة</p>}
            {filteredMsgs.map(m => (
          ...
          {selectedUser && (
            <div className="p-3 border-t border-gray-700/40 flex gap-2" style={{ background: 'rgba(10, 15, 30, 0.95)' }}>
              <input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()}
                className="flex-1 bg-gray-800/50 border border-gray-600/30 rounded-xl px-3 py-2.5 text-white text-xs placeholder-gray-500 focus:outline-none focus:border-blue-500/50" placeholder="رد..." />
...
export default function App() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      ...
      if (error) {
        ...
        notify('❌ فشل الاتصال بقاعدة البيانات: ' + error.message);
      } else {
        notify('✅ تم الاتصال بقاعدة البيانات بنجاح!');
      }
    };
    checkConnection();
  }, []);
  ...
  const handleWithdraw = () => {
    ...
    if (res.success) { notify(`تم إرسال طلب السحب! رسوم ${(amount * 0.05).toFixed(2)}$ ⏳`); setWithdrawAmount(''); setWalletAddress(''); refreshUser(); }
    else notify(res.error || 'خطأ');
  };
        ...
        {page === 'withdraw' && (
          <div className="max-w-md mx-auto space-y-5 animate-slide-up">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-xs">الحد الأدنى: $10 | رسوم: 5%</p>
            </div>
            <div className="glass-card rounded-2xl p-5">
              ...
              {withdrawAmount && parseFloat(withdrawAmount) >= 10 && (
                <div className="bg-gray-800/20 rounded-xl p-3 mb-4 space-y-1 animate-slide-up text-xs">
                  ...
                  <div className="flex justify-between"><span className="text-gray-500">رسوم 5%</span><span className="text-red-400">-${(parseFloat(withdrawAmount) * 0.05).toFixed(2)}</span></div>
                  <div className="border-t border-gray-800/30 pt-1 flex justify-between"><span className="text-gray-500">ستستلم</span><span className="text-green-400 font-bold">${(parseFloat(withdrawAmount) * 0.95).toFixed(2)}</span></div>
                </div>
              )}
              <p className="text-yellow-500/70 text-[10px] mb-4 flex items-start gap-1">
                <AlertTriangle size={12} className="shrink-0 mt-0.5" />
                <span>تأكد من صحة عنوان المحفظة. سيتم التحويل عبر شبكة ERC20 فقط.</span>
              </p>
              <button onClick={handleWithdraw} disabled={!walletAddress.trim()}
                      ...
                      <div key={tx.id} className="glass-card rounded-xl p-4 border border-yellow-500/10 animate-slide-up">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {tx.type === 'withdraw' && <span className="text-gray-600 text-[10px]">(رسوم: ${(tx.amount * 0.05).toFixed(2)})</span>}
