...
function CustomToast({ message, type, onClose }: { message: string; type: string; onClose: () => void }) {
  useEffect(() => {
  ...
  return (
    <div className="fixed top-4 right-4 z-50 animate-slide-in-right">
      <div className={`bg-gradient-to-r ${colors[type] || colors.info} rounded-xl shadow-2xl border p-4 min-w-[280px] max-w-md backdrop-blur-lg`}>
        ...
        <div className="overflow-y-auto max-h-[60vh] hide-scrollbar">
          {notifications.length === 0 ? (
            ...
            <div key={n.id} className={`p-4 border-b border-gray-800/40 flex items-start gap-3 transition-colors ${!n.read ? 'bg-blue-500/10' : 'hover:bg-gray-800/20'}`}>
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 border ${typeStyles[n.type] || typeStyles.info}`}>
...
function ChatWidget({ user, onClose }: { user: User; onClose: () => void }) {
  ...
  useEffect(() => {
    const load = () => {
      const all = store.getChatMessages();
      setMessages(all.filter((m: any) => m.from === user.id || m.to === user.id));
    };
    load();
...
function AdminChat({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden animate-slide-up h-[500px]">
