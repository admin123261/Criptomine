      ...
      <div className="relative glass-card-solid rounded-3xl p-8 max-w-sm w-full animate-scale-in text-center" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-8">
          ...
          <div className="bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 rounded-2xl p-5 mb-4">
            ...
            <p className="text-green-400 font-bold text-2xl mt-3">$5.00</p>
...
function NotificationCenter({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    const loadNotifications = () => {
      const notifsStr = localStorage.getItem(`notifications_${userId}`);
      if (notifsStr) {
        setNotifications(JSON.parse(notifsStr));
      }
    };
    loadNotifications();
...
function ChatWidget({ user, onClose }: { user: User; onClose: () => void }) {
  ...
  useEffect(() => {
    const loadMessages = () => {
      const allMessages = store.getChatMessages();
      const userMessages = allMessages.filter((m: any) => m.from === user.id || m.to === user.id);
...
function AdminChat({ onClose }: { onClose: () => void }) {
  ...
  const chatUsers = users.filter((u: any) => !u.isAdmin);
  const usersWithChats = chatUsers.filter((u: any) => messages.some((m: any) => m.from === u.id || m.to === u.id));
  const usersWithoutChats = chatUsers.filter((u: any) => !messages.some((m: any) => m.from === u.id || m.to === u.id));
  const filteredNewUsers = usersWithoutChats.filter((u: any) => u.username.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase()));
  const userMsgs = selectedUser ? messages.filter((m: any) => m.from === selectedUser || m.to === selectedUser) : [];
  const selectedUserInfo = selectedUser ? users.find((u: any) => u.id === selectedUser) : null;
    ...
    <div className="glass-card rounded-2xl overflow-hidden animate-slide-up">
      <div className="flex flex-col sm:flex-row h-[500px]">
        <div className={`${selectedUser ? 'hidden sm:block' : 'block'} w-full sm:w-1/3 border-b sm:border-b-0 sm:border-l border-gray-700/30 overflow-y-auto hide-scrollbar`}>
          ...
          {usersWithChats.map((u: any) => {
            const lastMsg = messages.filter((m: any) => m.from === u.id || m.to === u.id).sort((a: any, b: any) => b.date - a.date)[0];
            const unread = messages.filter((m: any) => m.from === u.id && m.to === 'admin001' && !m.read).length;
            return (
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 hide-scrollbar">
            {!selectedUser ? (
              ...
              <div key={m.id} className={`flex ${m.from === 'admin001' ? '